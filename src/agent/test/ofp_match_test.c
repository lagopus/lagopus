...
ofp_match_list_encode_wrap(struct channel *channel,
  ...
  *pbuf = pbuf_alloc(tlv_length);
  ...
  ret = ofp_match_list_encode(*pbuf, &match_list,
                              &total_len);
...
test_ofp_match_list_encode_null(void) {
  ...
  ret = ofp_match_list_encode(pbuf, NULL, &total_len);
  TEST_ASSERT_EQUAL_MESSAGE(LAGOPUS_RESULT_INVALID_ARGS, ret,
                            "ofp_match_list_encode error.");

  ret = ofp_match_list_encode(NULL, &match_list, &total_len);
  TEST_ASSERT_EQUAL_MESSAGE(LAGOPUS_RESULT_INVALID_ARGS, ret,
                            "ofp_match_list_encode error.");

  ret = ofp_match_list_encode(pbuf, &match_list, NULL);
  TEST_ASSERT_EQUAL_MESSAGE(LAGOPUS_RESULT_INVALID_ARGS, ret,
                            "ofp_match_list_encode error.");
