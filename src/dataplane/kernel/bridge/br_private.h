...
typedef int (*BR_FLSW) (struct net_device *dev, struct sk_buff *skbuff);
...
static inline struct net_bridge_port *br_port_get_rtnl(struct net_device
    *dev) {
...
extern void br_fdb_change_mac_address(struct net_bridge *br,
                                      const u8 *newaddr);
...
extern int br_ioctl_deviceless_stub(struct net *net, unsigned int cmd,
                                    void __user *arg);
...
extern struct net_bridge_mdb_entry *br_mdb_ip_get(
  ...
  struct br_ip *dst);
extern struct net_bridge_mdb_entry *br_multicast_new_group(
  struct net_bridge *br,
...
extern void br_stp_change_bridge_id(struct net_bridge *br,
