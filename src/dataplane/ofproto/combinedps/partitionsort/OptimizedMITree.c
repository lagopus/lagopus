...
void mit_insertion_r(optimized_mi_tree *tree,  rule *r) {
	...
	rb_tree_insert_with_path_compression(tree->root, r->fields, r->dim, 0, int_vector_to_array(tree->field_order), int_vector_size(tree->field_order), r);
	tree->num_rules++;
	tree->counter++;
}
...
void mit_deletion(optimized_mi_tree *tree,  rule *r, bool *priority_change) {
	...
	rb_tree_delete_with_path_compression(&tree->root, r->fields, 0, int_vector_to_array(tree->field_order), int_vector_size(tree->field_order), r->priority, &just_deleted_tree);
	//printf("<**\n");
}
