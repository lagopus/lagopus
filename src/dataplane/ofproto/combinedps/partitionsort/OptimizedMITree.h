...
static inline bool mit_can_insert_rule(optimized_mi_tree *tree,  rule *r){	
	return rb_tree_can_insert(tree->root, r->fields, r->dim, 0, int_vector_to_array(tree->field_order), int_vector_size(tree->field_order));
}
...
static inline rule_vector* mit_get_rules(optimized_mi_tree *tree) { return mit_serialize_into_rules(tree); }
