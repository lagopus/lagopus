...
add_flow_ipv4(struct flowinfo *, struct flow *);
static lagopus_result_t
del_flow_ipv4(struct flowinfo *, struct flow *);
static struct flow *
match_flow_ipv4(struct flowinfo *, struct lagopus_packet *, int32_t *);
static struct flow *
find_flow_ipv4(struct flowinfo *, struct flow *);
static void
destroy_flowinfo_ipv4(struct flowinfo *);
...
new_flowinfo_ipv4(void) {
  ...
  if (self != NULL) {
    ...
    self->add_func = add_flow_ipv4;
    self->del_func = del_flow_ipv4;
    self->match_func = match_flow_ipv4;
    self->find_func = find_flow_ipv4;
    self->destroy_func = destroy_flowinfo_ipv4;
  }
  return self;
}

static void
destroy_flowinfo_ipv4(struct flowinfo *self) {
...
static lagopus_result_t
add_flow_ipv4(struct flowinfo *self, struct flow *flow) {
  struct match *match;
  uint8_t proto;
...
static lagopus_result_t
del_flow_ipv4(struct flowinfo *self, struct flow *flow) {
  uint8_t proto;
  lagopus_result_t ret;
...
static struct flow *
match_flow_ipv4(struct flowinfo *self, struct lagopus_packet *pkt,
                int32_t *pri) {
  struct flowinfo *flowinfo;
...
static struct flow *
find_flow_ipv4(struct flowinfo *self, struct flow *flow) {
  uint8_t proto;
