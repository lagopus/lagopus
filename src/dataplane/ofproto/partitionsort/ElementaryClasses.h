...
typedef point* packet;//PACKET IS NOW A C ARRAY
...
typedef struct rule
{
	...
	//Used when converting a flow to a rule
	struct flow *master;
} rule;
...
static inline bool rule_matches_packet(struct rule *rule, const packet p){
	for (int i = 0; i < rule->dim; ++i) {
		if (p[i] < rule->range[i][low_dim] || p[i] > rule->range[i][high_dim]) return false;
	}
	return true;
}
...
static inline bool intersects_rules(const struct rule *rule1, const struct rule *rule2){
	for (int i = 0; i < rule1->dim; ++i) {
		if (rule1->range[i][high_dim] < rule2->range[i][low_dim] || rule1->range[i][low_dim] > rule2->range[i][high_dim]) return false;
	}
	return true;
}
...
static inline void print_rule(struct rule *rule){
	for (int i = 0; i < rule->dim; ++i) {
		printf("%u:%u ", rule->range[i][low_dim], rule->range[i][high_dim]);
	}
	printf("\n%lld\n",rule->priority);
	printf("\n");
}
...
static inline packet pbuf_create(const int PKT_SIZE){
...
void pbuf_init(const struct lagopus_packet *lp, uint8_t *fields, const int NUM_FIELDS, packet p);//Extracts the given fields from the packet and stores them in the packet, p
