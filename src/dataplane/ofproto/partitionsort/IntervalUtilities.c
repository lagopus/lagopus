...
bool iu_interval_is_identical(const rule *r1, const rule *r2) {
	for (int i = 0; i < r1->dim; ++i) {
		if (r1->range[i][WI_LOW] != r2->range[i][WI_LOW]) return false;
		if (r1->range[i][WI_HIGH] != r2->range[i][WI_HIGH]) return false;
	}
	return true;
}
...
void create_unique_interval(rule_vector *rules, int field, weighted_interval ***intervals, int *num_intervals) {
	...
	for(size_t i = 0; i < num_rules; ++i){
		build_interval(rules_array[i]->range[field][WI_LOW], rules_array[i]->range[field][WI_HIGH], i, &sorted_rules[i]);
	}
...
int weighted_pair_compare(const void *lhs, const void *rhs);
int weighted_pair_compare(const void *lhs, const void *rhs){
