...
void ParseRange(unsigned *range, const char *text) {
...
rule_vector * ReadFilterFileMSU(const char *filename)
{
	...
	while ((read = getline(&line, &len, fp)) != -1) {
		...
		rule *temp_rule = rule_create(dim);
		...
		for (size_t i = 0; i < dim; i++)
		{
			...
			if (IsPrefix(temp_rule->range[i][0], temp_rule->range[i][1])) {
				temp_rule->prefix_length[i] = PrefixLength(temp_rule->range[i][0], temp_rule->range[i][1]);
			}
