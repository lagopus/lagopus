...
cache_enable(struct lagopus_packet *pkt) {
  pkt->cache = init_flowcache(FLOWCACHE_HASHMAP_NOLOCK);
}

void
destroy_packet(struct lagopus_packet *pkt) {
  if (pkt->cache != NULL) {
    fini_flowcache(pkt->cache);
  }
  lagopus_packet_free(pkt);
}

void
flow_benchmark(struct lagopus_packet *pkt, time_t sec) {
...
test_MPLS_1M_entries_flow_benchmark(void) {
  ...
  printf("\n*** 1M entries (MPLS label = 1M -> 1)\n");
  mpls_flow_add(1000 * 1000, 1);
  pkt = build_mpls_packet(1, 100);
  flow_benchmark(pkt, 1);
  printf("\n*** cached 1M entries (MPLS label = 1M -> 1)\n");
  cache_enable(pkt);
  flow_benchmark(pkt, 1);
  destroy_packet(pkt);
}
...
void
test_IPV4_SRC_65536_entries_flow_benchmark(void) {
  struct lagopus_packet *pkt;

  printf("\n*** 65536 entries (IPV4_SRC 192.168.0.0-192.168.255.255)\n");
  ipsrc_flow_add(IPADDR(192,168,0,0), IPADDR(192,168,255,255), 32);
  pkt = build_ip_packet(1, IPADDR(192,168,1,1), IPADDR(192,168,1,2), 0);
  flow_benchmark(pkt, 1);
  printf("\n*** cached 65536 entries (IPV4_SRC 192.168.0.0-192.168.255.255)\n");
  cache_enable(pkt);
  flow_benchmark(pkt, 1);
  destroy_packet(pkt);
}
...
void
test_IPV4_DST_65536_entries_flow_benchmark(void) {
  struct lagopus_packet *pkt;

  printf("\n*** 65536 entries (IPV4_DST 192.168.0.0-192.168.255.255)\n");
  ipdst_flow_add(IPADDR(192,168,0,0), IPADDR(192,168,255,255), 32);
  pkt = build_ip_packet(1, IPADDR(192,168,1,1), IPADDR(192,168,1,2), 0);
  flow_benchmark(pkt, 1);
  printf("\n*** cached 65536 entries (IPV4_DST 192.168.0.0-192.168.255.255)\n");
  cache_enable(pkt);
  flow_benchmark(pkt, 1);
  destroy_packet(pkt);
}
