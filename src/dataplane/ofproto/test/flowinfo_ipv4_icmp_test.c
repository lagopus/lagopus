...
/*
 * The ICMP header matches are stored in the leaf basic flowinfo.  del
 * -> 2*add -> 3*del is hence sufficient.
 */

void
test_flowinfo_ipv4_icmp_type_adddel(void) {
  size_t s;

  TEST_ASSERT_OBJECTS();

  /* Add the ICMP type matches. */
  for (s = 0; s < ARRAY_LEN(test_flow); s++) {
    FLOW_ADD_ICMP_TYPE_MATCH(test_flow[s], TEST_ICMP_TYPE(s));
  }
