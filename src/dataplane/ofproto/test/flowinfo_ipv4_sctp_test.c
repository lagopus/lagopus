...
#define FLOW_ADD_UT_PREREQUISITE(_x)		\
  FLOW_ADD_IPV4_SCTP_PREREQUISITE(_x)
#define	TEST_ASSERT_FLOW_MATCH_UT_PREREQUISITE(_x)	\
  TEST_ASSERT_FLOW_MATCH_IPV4_SCTP_PREREQUISITE(_x)
...
/*
 * The SCTP header matches are stored in the leaf basic flowinfo.  del
 * -> 2*add -> 3*del is hence sufficient.
 */

void
test_flowinfo_ipv4_sctp_src_adddel(void) {
  size_t s;

  TEST_ASSERT_OBJECTS();

  /* Add the SCTP source port matches. */
  for (s = 0; s < ARRAY_LEN(test_flow); s++) {
    FLOW_ADD_SCTP_SRC_MATCH(test_flow[s], TEST_L4_PORT(s));
  }
  ...
  for (s = 0; s < ARRAY_LEN(test_flow); s++) {
    ...
    FLOW_ADD_IPV4_SCTP_PREREQUISITE(test_flow[s]);
  }
}

void
test_flowinfo_ipv4_sctp_dst_adddel(void) {
  size_t s;

  TEST_ASSERT_OBJECTS();

  /* Add the SCTP destination port matches. */
  for (s = 0; s < ARRAY_LEN(test_flow); s++) {
    FLOW_ADD_SCTP_DST_MATCH(test_flow[s], TEST_L4_PORT(s));
  }
