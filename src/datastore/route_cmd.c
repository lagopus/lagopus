...
/**
 *      @file   route_cmd.c
 *      @brief  DSL command parser for L3 route table management.
 */
  ...
  if (argv != NULL && out_configs != NULL && result != NULL) {
    if (*(*argv + 1) != NULL) {
      ...
      if (IS_VALID_STRING(*(*argv)) == true) {
        ret = route_cmd_dump_tmp_dir_set(*(*argv));
      ...
      if ((ret = route_cmd_dump_tmp_dir_get(&path)) ==
          LAGOPUS_RESULT_OK) {
...
dump_sub_cmd_parse(datastore_interp_t *iptr,
  ...
  if (argv != NULL && conf != NULL &&
    ...
    if (ret == LAGOPUS_RESULT_OK) {
      /* dump route. */
      if ((ret = route_cmd_dump_thread_start(conf,
                                            iptr,
                                            result)) !=
          LAGOPUS_RESULT_OK) {
        ret = datastore_json_result_string_setf(
                result,ret,
                "Can't start route dump thread.");
      }
    }
  } else {
    ret = datastore_json_result_set(result,
...
STATIC lagopus_result_t
route_cmd_parse(datastore_interp_t *iptr,
               datastore_interp_state_t state,
               size_t argc, const char *const argv[],
               lagopus_hashmap_t *hptr,
  ...
  char *fullname = NULL;
  char *str = NULL;
  route_conf_t conf = {NULL, OFPTT_ALL};
...
initialize_internal(void) {
  ...
  if ((ret = datastore_interp_register_command(&s_interp, CONFIGURATOR_NAME,
             CMD_NAME, route_cmd_parse)) !=
      LAGOPUS_RESULT_OK) {
    lagopus_perror(ret);
    goto done;
  }

  if ((ret = route_cmd_dump_initialize()) != LAGOPUS_RESULT_OK) {
    goto done;
  }
