...
test_policer_conf_duplicate(void) {
  ...
  rc = policer_conf_create(&src_conf, src_fullname);
  TEST_ASSERT_EQUAL(LAGOPUS_RESULT_OK, rc);
  TEST_ASSERT_NOT_NULL_MESSAGE(src_conf,
                               "conf_create() will create new policer");

  // Normal case
  {
    rc = policer_conf_duplicate(src_conf, &dst_conf, dst_fullname);
    TEST_ASSERT_EQUAL(LAGOPUS_RESULT_OK, rc);
    result = policer_conf_equals(src_conf, dst_conf);
    TEST_ASSERT_TRUE(result);
  }

  // Abnormal case
  {
    result = policer_conf_equals(src_conf, NULL);
    TEST_ASSERT_FALSE(result);

    result = policer_conf_equals(NULL, dst_conf);
    TEST_ASSERT_FALSE(result);
  }
