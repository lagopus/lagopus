...
#define FLOWDB_RWLOCK_INIT(lock)
#define FLOWDB_RWLOCK_RDLOCK(lock)  do {				\
    (void) lock;							\
    rte_rwlock_read_lock(&dpmgr_lock);					\
  } while (0)
#define FLOWDB_RWLOCK_WRLOCK(lock)  do {				\
    (void) lock;							\
    rte_rwlock_write_lock(&dpmgr_lock);					\
  } while (0)
#define FLOWDB_RWLOCK_RDUNLOCK(lock) do {				\
    (void) lock;							\
    rte_rwlock_read_unlock(&dpmgr_lock);				\
  } while (0)
#define FLOWDB_RWLOCK_WRUNLOCK(lock) do {				\
    (void) lock;							\
    rte_rwlock_write_unlock(&dpmgr_lock);				\
  } while (0)
#else
pthread_rwlock_t dpmgr_lock;
#define FLOWDB_RWLOCK_INIT(lock) do {                                   \
    (void) lock;							\
...
flowdb_rdlock(struct flowdb *flowdb) {
  FLOWDB_RWLOCK_RDLOCK(&flowdb->rwlock);
  return 0;
}
...
flowdb_wrlock(struct flowdb *flowdb) {
  FLOWDB_RWLOCK_WRLOCK(&flowdb->rwlock);
  return 0;
}
...
flowdb_rdunlock(struct flowdb *flowdb) {
  FLOWDB_RWLOCK_RDUNLOCK(&flowdb->rwlock);
  return 0;
}
...
flowdb_wrunlock(struct flowdb *flowdb) {
  FLOWDB_RWLOCK_WRUNLOCK(&flowdb->rwlock);
  return 0;
}
