...
void
test_IfTable_get_ifAdminStatus(void) {
  ...
  ret_val = get_ifAdminStatus(dctx1, &ret_len);
  TEST_ASSERT_NOT_NULL(ret_val);
  TEST_ASSERT_EQUAL_INT32(VALUE_ifAdminStatus_1, *ret_val);
  ...
  TEST_ASSERT_NULL(dctx3);
  ifTable_loop_free(lctx, NULL);
  ret_val = get_ifAdminStatus(dctx2, &ret_len);
  TEST_ASSERT_NOT_NULL(ret_val);
  TEST_ASSERT_EQUAL_INT32(VALUE_ifAdminStatus_2, *ret_val);
  TEST_ASSERT_EQUAL_UINT64(sizeof(*ret_val), ret_len);
  ifTable_data_free(dctx2, NULL);
}
