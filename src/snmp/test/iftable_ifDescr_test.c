...
test_IfTable_get_ifDescr(void) {
  ...
  const char value1[] = VALUE_ifDescr_1;
  const char value2[] = VALUE_ifDescr_2;
  ...
  ret_val = get_ifDescr(dctx1, &ret_len);
  TEST_ASSERT_NOT_NULL(ret_val);
  TEST_ASSERT_EQUAL_UINT64(sizeof(value1) - 1, ret_len);
  TEST_ASSERT_EQUAL_UINT8_ARRAY(value1, ret_val, sizeof(value1) - 1);
  ...
  TEST_ASSERT_NULL(dctx3);
  ifTable_loop_free(lctx, NULL);
  ret_val = get_ifDescr(dctx2, &ret_len);
  TEST_ASSERT_NOT_NULL(ret_val);
  TEST_ASSERT_EQUAL_UINT64(sizeof(value2) - 1, ret_len);
  TEST_ASSERT_EQUAL_UINT8_ARRAY(value2, ret_val, sizeof(value2) - 1);
  ifTable_data_free(dctx2, NULL);
}
