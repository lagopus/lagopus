...
void
test_IfTable_get_ifOutNUcastPkts(void) {
  ...
  ret_val = get_ifOutNUcastPkts(dctx1, &ret_len);
  TEST_ASSERT_NULL(ret_val); /* deprecated! */
  ...
  TEST_ASSERT_NULL(dctx3);
  ifTable_loop_free(lctx, NULL);
  ret_val = get_ifOutNUcastPkts(dctx2, &ret_len);
  TEST_ASSERT_NULL(ret_val);
  ifTable_data_free(dctx2, NULL);
}
