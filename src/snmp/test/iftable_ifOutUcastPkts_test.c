...
void
test_IfTable_get_ifOutUcastPkts(void) {
  ...
  ret_val = get_ifOutUcastPkts(dctx1, &ret_len);
  TEST_ASSERT_NOT_NULL(ret_val);
  TEST_ASSERT_EQUAL_UINT32(VALUE_ifOutUcastPkts_1, *ret_val);
  ...
  TEST_ASSERT_NULL(dctx3);
  ifTable_loop_free(lctx, NULL);
  ret_val = get_ifOutUcastPkts(dctx2, &ret_len);
  TEST_ASSERT_NOT_NULL(ret_val);
  TEST_ASSERT_EQUAL_UINT32(VALUE_ifOutUcastPkts_2, *ret_val);
  TEST_ASSERT_EQUAL_UINT64(sizeof(*ret_val), ret_len);
  ifTable_data_free(dctx2, NULL);
}
